...
- `StrategyError`: recoverable per tick; `strategy.evaluate(...)` failures produce structured `StrategyError` metadata, safe hold / `strategy_error`, and must not open a position. Repeated-error breaker remains future work.
...
- Future wiring target: call these helpers at execution/user-stream/state-store mutation boundaries after ownership is narrowed enough to preserve behavior safely.
