const refs = {
  ...
  focusArchitectNote: document.getElementById("focus-architect-note"),
...
function formatArchitectBadge(architect) {
  ...
  if (!architect.ready) return "Architect warm-up";
...
function renderFocusMeta() {
  ...
  refs.focusMeta.innerHTML = [
    ...
    { label: "Strength", value: architectReady ? formatNumber(architect?.decisionStrength, 2) : "warming" },
    { label: "Agreement", value: architectReady ? formatUnitPercent(architect?.signalAgreement) : "warming" },
    { label: "Maturity", value: architect ? formatUnitPercent(architect?.contextMaturity ?? architect?.maturity) : "n/a" },
      ...
      <tr class="bot-row ${bot.symbol === state.focusSymbol ? "is-selected" : ""}" data-symbol="${escapeHtml(bot.symbol)}">
        ...
        <td>
          <div class="cell-stack">
            ...
            <span class="muted">${escapeHtml(`${strategyNote} | sync ${humanizeToken(bot.syncStatus || "pending")}`)}</span>
          </div>
        </td>
